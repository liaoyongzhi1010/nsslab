...
def setup_function():
    ...
    login = client.post("/api/auth/login", json={"username": "admin", "password": "Admin-Test-Password-2026!"})
...
def test_authentication_roles_logout_and_project_ownership():
    ...
    wrong = client.post("/api/auth/login", json={"username": "student", "password": "definitely-wrong"})
    assert wrong.status_code == 401

    student_login = client.post("/api/auth/login", json={"username": "student", "password": "Student-Test-Password-2026!"})
    ...
    assert client.put("/api/skills/crypto_explain", json={"description": "学生不应修改全局技能", "steps": ["blocked"], "enabled": True}).status_code == 403
    assert client.get("/api/admin/users").status_code == 403
    assert client.post("/api/auth/logout").status_code == 204
    assert client.get(f"/api/projects/{student_project_id}").status_code == 401

    admin_login = client.post("/api/auth/login", json={"username": "admin", "password": "Admin-Test-Password-2026!"})
    assert admin_login.status_code == 200
    assert admin_login.json()["user"]["role"] == "admin"
    assert client.get(f"/api/projects/{student_project_id}").status_code == 200
    created_user = client.post("/api/admin/users", json={"username": "student2", "display_name": "学生二号", "password": "Student-Two-Password-2026!", "role": "student"})
    assert created_user.status_code == 201

    client.post("/api/auth/logout")
    assert client.post("/api/auth/login", json={"username": "student2", "password": "Student-Two-Password-2026!"}).status_code == 200
    assert client.get(f"/api/projects/{student_project_id}").status_code == 403
    assert client.patch(f"/api/projects/{student_project_id}", json={"ended": True}).status_code == 403


def test_public_registration_creates_only_student_and_starts_session():
    client.cookies.clear()
    registered = client.post("/api/auth/register", json={
        "username": "new.student", "display_name": "新同学", "password": "New-Student-Password-2026!",
    })
    ...
    assert client.post("/api/projects", json={"name": "注册用户项目"}).status_code == 201

    duplicate = client.post("/api/auth/register", json={
        "username": "new.student", "display_name": "重复账号", "password": "Another-Password-2026!",
    })
    assert duplicate.status_code == 400
    injected_role = client.post("/api/auth/register", json={
        "username": "fake-admin", "display_name": "越权账号", "password": "Fake-Admin-Password-2026!", "role": "admin",
    })
...
def test_restart_moves_current_experiment_to_history_and_restore_is_recoverable():
    client.cookies.clear()
    assert client.post("/api/auth/login", json={"username": "student", "password": "Student-Test-Password-2026!"}).status_code == 200
    first = client.post("/api/projects", json={"name": "第一次 RAG 实验"}).json()
    built = client.post(
        "/api/kb/build",
        json={"project_id": first["id"], "document_ids": ["aes", "rsa"], "chunk_size": 256, "overlap": 32},
    )
    ...
    assert client.post(
        "/api/kb/build",
        json={"project_id": first["id"], "document_ids": ["aes"], "chunk_size": 256, "overlap": 32},
    ).status_code == 400

    renamed = client.patch(f"/api/projects/{first['id']}", json={"name": "第一次实验（已复盘）"})
...
def test_account_is_temporarily_locked_after_repeated_failures():
    created = client.post("/api/admin/users", json={
        "username": "lock-test", "display_name": "锁定测试账号",
        "password": "Lock-Test-Password-2026!", "role": "student",
    })
    assert created.status_code == 201
    client.post("/api/auth/logout")
    for _ in range(5):
        response = client.post("/api/auth/login", json={"username": "lock-test", "password": "wrong-password"})
        assert response.status_code == 401
    locked = client.post("/api/auth/login", json={"username": "lock-test", "password": "Lock-Test-Password-2026!"})
...
def create_project_and_kb():
    ...
    response = client.post(
        "/api/kb/build",
        json={"project_id": project["id"], "document_ids": ["aes", "rsa", "tee", "he", "mpc"], "chunk_size": 512, "overlap": 64},
    )
...
def test_bootstrap_and_build_knowledge_base():
    ...
    assert next(row for row in body["documents"] if row["id"] == "nist_fips203")["source_type"] == "NIST 正式标准"
    assert next(row for row in body["documents"] if row["id"] == "gmit_2023_revision")["source_type"] == "国家密码管理局官方公告"
    assert next(row for row in body["documents"] if row["id"] == "kunpeng_secgear_dev")["local_original"].endswith(".html.txt")
...
def test_rsa_retrieval_and_rag_trace():
    ...
    search = client.post("/api/kb/search", json={"project_id": project_id, "query": query, "top_k": 5, "threshold": 0.02})
    assert search.status_code == 200
    assert any("RSA" in row["document_title"] for row in search.json()["results"][:3])
    rag = client.post(
        "/api/rag/compare",
        json={"project_id": project_id, "query": query, "top_k": 5, "threshold": 0.02, "rerank_enabled": True, "rerank_top_n": 3},
    )
    body = rag.json()
...
def test_private_evidence_benchmark_makes_rag_gain_measurable():
    ...
    built = client.post(
        "/api/kb/build",
        json={"project_id": project["id"], "document_ids": ["hailan_crypto_manual"], "chunk_size": 512, "overlap": 64},
    )
    assert built.status_code == 200
    benchmark = next(row for row in client.get("/api/bootstrap").json()["rag_benchmarks"] if row["id"] == "private_incident")
    response = client.post(
        "/api/rag/compare",
        json={
            "project_id": project["id"], "query": benchmark["question"], "benchmark_id": benchmark["id"],
            "top_k": 5, "threshold": 0.0, "rerank_enabled": True, "rerank_top_n": 3,
        },
    )
    assert response.status_code == 200
    body = response.json()
...
def test_standard_benchmark_exposes_verifiable_primary_source():
    ...
    client.post(
        "/api/kb/build",
        json={"project_id": project["id"], "document_ids": ["nist_fips203"], "chunk_size": 512, "overlap": 64},
    )
    benchmark = next(row for row in client.get("/api/bootstrap").json()["rag_benchmarks"] if row["id"] == "fips203_sizes")
    response = client.post(
        "/api/rag/compare",
        json={"project_id": project["id"], "query": benchmark["question"], "benchmark_id": benchmark["id"], "top_k": 5, "threshold": 0.0},
    )
    assert response.status_code == 200
    body = response.json()
    assert body["benchmark"]["rag_score"] == 100
    assert body["rag"]["citations"][0]["source_url"] == "https://doi.org/10.6028/NIST.FIPS.203"
...
def test_2026_signature_benchmark_is_grounded_in_current_nist_report():
    ...
    client.post(
        "/api/kb/build",
        json={"project_id": project["id"], "document_ids": ["nist_ir8610_2026"], "chunk_size": 512, "overlap": 64},
    )
    benchmark = next(row for row in client.get("/api/bootstrap").json()["rag_benchmarks"] if row["id"] == "nist_signatures_2026")
    response = client.post(
        "/api/rag/compare",
        json={"project_id": project["id"], "query": benchmark["question"], "benchmark_id": benchmark["id"], "top_k": 5, "threshold": 0.0},
    )
    ...
    assert body["benchmark"]["base_score"] == 0
    assert body["benchmark"]["rag_score"] == 100
    assert body["rag"]["citations"][0]["source_url"] == "https://doi.org/10.6028/NIST.IR.8610"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("phytium_security_stack", ["phytium_phytee_platform", "phytium_tee_architecture"]),
    ],
)
def test_domestic_standard_and_tee_benchmarks_show_grounded_gain(benchmark_id, document_ids):
    project = client.post("/api/projects", json={"name": f"国产专题核验 {benchmark_id}"}).json()
    built = client.post(
        "/api/kb/build",
        json={"project_id": project["id"], "document_ids": document_ids, "chunk_size": 512, "overlap": 64},
    )
    assert built.status_code == 200
    benchmark = next(row for row in client.get("/api/bootstrap").json()["rag_benchmarks"] if row["id"] == benchmark_id)
    response = client.post(
        "/api/rag/compare",
        json={
            "project_id": project["id"], "query": benchmark["question"], "benchmark_id": benchmark_id,
            "top_k": 8, "threshold": -0.1, "rerank_enabled": True, "rerank_top_n": 6,
        },
    )
    assert response.status_code == 200
    body = response.json()
...
def test_benchmark_reports_missing_required_document_and_rejects_edited_question():
    ...
    missing = client.post(
        "/api/rag/compare",
        json={"project_id": project_id, "query": benchmark["question"], "benchmark_id": benchmark["id"], "top_k": 3, "threshold": 0.0},
    )
    assert missing.status_code == 200
    assert missing.json()["benchmark"]["missing_document_ids"] == ["hailan_crypto_manual"]
    edited = client.post(
        "/api/rag/compare",
        json={"project_id": project_id, "query": benchmark["question"] + "（已改）", "benchmark_id": benchmark["id"]},
    )
...
def test_agent_selection_trace_and_safe_tools():
    ...
    response = client.post(
        "/api/agents/run",
        json={"project_id": project_id, "query": "我需要在云服务器处理敏感数据，请比较 HE、MPC、TEE 并给出选型建议。", "memory_enabled": True},
    )
    body = response.json()
    assert response.status_code == 200
    ...
    calculator = client.post("/api/tools/calculator/run", json={"arguments": {"expression": "20 / 1.25"}})
    assert calculator.json()["output"]["value"] == 16
    blocked = client.post("/api/tools/calculator/run", json={"arguments": {"expression": "__import__('os').system('id')"}})
...
def test_multiple_projects_can_each_build_same_agent():
    ...
    assert client.patch(f"/api/projects/{first_id}", json={"ended": False}).status_code == 200
    first = client.post("/api/agents/run", json={"project_id": first_id, **payload})
    assert client.patch(f"/api/projects/{second_id}", json={"ended": False}).status_code == 200
...
def test_report_after_complete_flow():
    ...
    client.post(
        "/api/rag/compare",
        json={"project_id": project_id, "query": "TrustZone 与 SGX 的隔离机制有什么区别？", "top_k": 5, "threshold": 0.0},
    )
    client.post(
        "/api/agents/run",
        json={"project_id": project_id, "query": "比较 HE、MPC、TEE 并给出选型建议", "memory_enabled": True},
    )
...
def test_rich_text_observation_is_sanitized_persisted_and_exported_with_formatting():
    ...
    saved = client.put(f"/api/reports/{project_id}/observation", json={"html": observation_html})
    ...
    assert any(run.bold and "Top-K" in run.text for paragraph in paragraphs for run in paragraph.runs)
    assert any(paragraph.style.name == "List Bullet" and "继续控制上下文噪声" in paragraph.text for paragraph in paragraphs)
...
def test_upload_text_and_code_then_build_knowledge_base():
    ...
    text_upload = client.post(
        ...
        files={"file": ("my-notes.txt", "格密码的安全性通常依赖格上困难问题。", "text/plain")},
    )
    ...
    code_upload = client.post(
        "/api/documents/upload",
        data={"project_id": project["id"]},
        files={"file": ("rsa_demo.py", b"def mod_pow(a, e, n):\n    return pow(a, e, n)\n", "text/x-python")},
    )
    ...
    built = client.post(
        "/api/kb/build",
        json={"project_id": project["id"], "document_ids": [text_upload.json()["id"], code["id"]], "chunk_size": 128, "overlap": 0},
    )
    assert built.status_code == 200
    assert built.json()["document_count"] == 2
    assert any("代码符号：mod_pow" == chunk["section"] for chunk in built.json()["chunks"])
...
def test_upload_pdf_extracts_page_text():
    ...
    stream = StreamObject()
    stream.set_data(b"BT /F1 12 Tf 72 720 Td (RSA hybrid encryption uses AES for bulk data.) Tj ET")
    ...
    assert body["pages"] == 1
    detail = client.get(f"/api/documents/{body['id']}?project_id={project['id']}").json()
