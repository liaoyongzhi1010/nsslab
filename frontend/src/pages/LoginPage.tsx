  ...
  return <main className="login-page">
    ...
    <section className="login-form-side">
      <form className="login-card" onSubmit={submit}>
        ...
        <label><span>用户名</span><div className="login-input"><UserRound /><input id="username" name="username" autoFocus autoComplete="username" aria-label="用户名" placeholder="字母、数字、点、横线或下划线" pattern="[A-Za-z0-9_.-]+" minLength={2} maxLength={64} value={username} onChange={(event) => setUsername(event.target.value)} required /></div></label>
        <label><span>密码</span><div className="login-input"><KeyRound /><input id="password" name="password" type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} aria-label="密码" placeholder={mode === "login" ? "请输入密码" : "至少 8 个字符"} minLength={8} maxLength={256} value={password} onChange={(event) => setPassword(event.target.value)} required /></div></label>
